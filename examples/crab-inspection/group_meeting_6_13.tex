...
\pagestyle{fancy}
\fancyhf{}
\fancyfoot[L]{\footnotesize\textcolor{black!50}{蟹公寓巡检系统 \,$\cdot$\, 2026.06.13}}
...
\begin{document}
% (header/footer handled globally via fancyhdr pagestyle)
...
\node[comp, fill=wingreen!8, draw=wingreen, minimum width=1.5cm, minimum height=0.7cm,
      font=\scriptsize\bfseries] (s1) at (5.2, -1.0) {S1\\稳定};
\node[comp, fill=wingreen!8, draw=wingreen, minimum width=1.5cm, minimum height=0.7cm,
      font=\scriptsize\bfseries] (s2) at (7.4, -1.0) {S2\\居中};
\node[comp, fill=wingreen!8, draw=wingreen, minimum width=1.5cm, minimum height=0.7cm,
      font=\scriptsize\bfseries] (s3) at (9.6, -1.0) {S3\\靠近};
\node[comp, fill=accent!10, draw=accent, minimum width=1.5cm, minimum height=0.7cm,
      font=\scriptsize\bfseries] (s4) at (11.8, -1.0) {S4\\停留};
...
\node[comp, fill=wingreen!8, draw=wingreen, minimum width=1.7cm, minimum height=0.7cm,
      font=\scriptsize\bfseries] (s5) at (7.4, -2.9) {S5\\后退换框};
\node[comp, fill=accentorange!10, draw=accentorange, minimum width=1.3cm, minimum height=0.7cm,
      font=\scriptsize\bfseries] (s6) at (11.8, -2.9) {DONE};
...
\foreach \i/\x in {1/-8.0, 2/-5.3, 3/-2.6, 4/0.1, 5/2.8, 6/5.5} {
  \node[tag, fill=black!50] at (\x, -1.0) {Step \i};
}
...
\textbf{标记} & \textbf{设计要点} \\
\midrule
\textcolor{wingreen}{\textbf{+}} & 每步注册下一步回调后立即返回 \\
\textcolor{wingreen}{\textbf{+}} & Executor 每步之间空闲，能处理响应 \\
\textcolor{wingreen}{\textbf{+}} & 配合多线程 Executor + 可重入回调组 \\
\textcolor{accent}{\textbf{$\times$}} & 回调中永远不要阻塞等待 \\
\bottomrule
\end{tabular}
\end{center}
